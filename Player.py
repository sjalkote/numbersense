...
class Player:
    ...
    def fix_username(username: str) -> (str, bool):
        ...
        if not username.isspace() and username != '':
            ...
            value = (unicodedata.normalize("NFKD", username).encode(
    ...
    def checkPassword(username: str, provided_password: str) -> bool:
        try:
            with open(f"users/{username}.json") as f:
                data = json.load(f)
                return utils.check_password(provided_password,
                                            data["password"])
        except FileNotFoundError:
            print(
                f"{C.RED}Couldn't find player data for {C.BLUE}{username}{C.RED} when checking password."
            )
        return False
    ...
    def __init__(self, username, mode, password=None):
        ...
        # USERNAME HANDLING ----------------------------------------------------
        new_account = False
        self.name, self.is_guest = self.fix_username(username)
        if self.name != username:
            print(
                f"{C.YELLOW}Username corrected, changed to: {C.CYAN}{self.name}"
            )
        del username
        # ACCOUNT LOGIN / CREATION ----------------------------------------------------

        # Entering Password ---------------
        if utils.check_if_user_data_present(self.name):
            # Check to make sure a user with this name does not already exist
            tries: int = 0
            ...
            while not login_successful:
                ...
                password = getpass()
                if Player.checkPassword(self.name, password):
                    print(
                        f"{C.GREEN}Correct password.{Style.RESET_ALL} Welcome back, {C.MAGENTA}{self.name}"
                    )
        ...
        if new_account:
            ...
            if os.path.isfile(self._USER_DATA_FILE) and os.stat(
                    self._USER_DATA_FILE).st_size != 0:
                with open(self._USER_DATA_FILE, "r") as score_file:
                    ...
                    try:
                        ...
                        print(
                            f"{C.GREEN}Retrieved High Scores!{Style.RESET_ALL}"
                        )
                        print(
                            tabulate(
                                [[score for score in self.highscore.values()]],
                                headers=[
                                    mode for mode in self.highscore.keys()
                                ],
                                tablefmt='orgtbl'))
                ...
                print(
                    f"{C.RED}No save data found! Creating basic data file: {C.CYAN}{self.generate_empty_data_file()}"
                )
    ...
    def get_high_scores(self):
        with open(self._USER_DATA_FILE, "r") as score_file:
            ...
            try:
                ...
                print(
                    tabulate([[score for score in self.highscore.values()]],
    ...
    def changePassword(self, new_password):
        with open(self._USER_DATA_FILE, 'r') as data_file:
            ...
            new_password = utils.encrypt_password(new_password,
                                                  return_as_str=True)
